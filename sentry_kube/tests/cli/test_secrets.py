...
from sentry_kube.cli.secrets import (
    ...
    merge_userlists,
    pg_scram_sha256,
    upload_plaintext_to_k8s_secret,
)


@mock.patch(
    "os.urandom",
    return_value=b"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f",
)
def test_pg_scram_sha256(mock_urandom):
    result = pg_scram_sha256("helloworld")
    assert (
        result
        == "SCRAM-SHA-256$4096:AAECAwQFBgcICQoLDA0ODw==$fz0NTQhi0PzVISJdN1T1uSGlWXfLKg4O6uedGG0EfBQ=:hoPVYNrCgxf3TtXeuKoFHAtGQy4G+fMQvjNMs76hw+k="
    )
...
class TestUserlist:
    ...
    def test_merge_userlists_add_new_user(self, mock_stdout):
        result = merge_userlists(
            self.userlist_b64, {"user3": {"scram": "SCRAM-SHA-256$333"}}, "example"
        )
        output = mock_stdout.getvalue()

        assert (
            base64.b64decode(result)
            == b'"user1" "SCRAM-SHA-256$111"\n"user2" "SCRAM-SHA-256$222"\n"user3" "SCRAM-SHA-256$333"\n'
        )
...
def test_secret_does_not_exist(mock_stdout):
    ...
    assert (
        core_api_mock.create_namespaced_secret.call_args[1]["body"].metadata["name"]
        == "example"
    )
